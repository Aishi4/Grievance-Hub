...
export default function UploadPhoto() {
  ...
  return (
    <div className="max-w-md mx-auto p-6 shadow-lg rounded-2xl border">
      ...
      {step === 1 && (
        <div>
          ...
          <input type="file" onChange={(e) => setFile(e.target.files[0])} className="mb-4 border p-2 w-full" />
      ...
      {step === 2 && (
        <div>
          ...
          <input type="text" placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} className="mb-4 border p-2 w-full" />
      ...
      {step === 3 && (
        <div>
          <h2 className="text-xl font-bold mb-4">Step 3: Add Location</h2>
          <input type="text" placeholder="Location" value={location} onChange={(e) => setLocation(e.target.value)} className="mb-4 border p-2 w-full" />
          <div className="flex justify-between mt-4">
      ...
      {step === 4 && (
        <div>
          ...
          <p>{uploadStatus}</p>
          <button onClick={() => setStep(1)} className="mt-4 bg-blue-500 text-white px-4 py-2 rounded">Upload Another</button>
        </div>
      )}
    </div>
  );
