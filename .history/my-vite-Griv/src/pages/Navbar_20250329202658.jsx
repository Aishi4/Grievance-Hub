...
<div className="hidden md:flex items-center space-x-6">
  {navItems.map((item, index) => (
    <motion.div key={item.to} onHoverStart={() => setIsHovered(index)} onHoverEnd={() => setIsHovered(null)} className="relative">
      <Link to={item.to} className={`flex items-center space-x-2 py-2 px-3 rounded-lg ${location.pathname === item.to && !item.special ? "bg-emerald-700/50" : ""} ${item.special ? "bg-white text-emerald-600 px-4 py-2 rounded-full shadow-md hover:shadow-lg hover:bg-emerald-50" : "text-white hover:text-emerald-200"} transition-all duration-300`}>
        <item.icon size={18} className={item.special ? "text-emerald-500" : ""} />
        <span className="font-medium">{item.label}</span>
      </Link>
    </motion.div>
  ...
  {user ? (
    ...
    <div className="flex items-center space-x-4">
      <img src={user.photoURL || "/default-avatar.png"} alt="User" className="w-10 h-10 rounded-full border border-gray-300" />
      <span className="text-white font-medium">{user.displayName || "User"}</span>
      <button onClick={handleLogout} className="flex items-center space-x-2 bg-red-500 text-white px-4 py-2 rounded-full font-semibold shadow-md hover:bg-red-400 transition-all duration-300">
        <LogOut size={18} />
        <span>Logout</span>
      </button>
    </div>
    ...
    <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
      <Link to="/login" className="flex items-center space-x-2 bg-emerald-500 text-white px-4 py-2 rounded-full font-semibold shadow-md hover:bg-emerald-400 transition-all duration-300">
        <LogIn size={18} />
        <span>Login</span>
      </Link>
    </motion.div>
